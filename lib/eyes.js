 ...
 * @method stylize
 * @param  {String} str
 * @param  {String} style Valid chalk style
 * @see https://github.com/chalk/ansi-styles
 */
eyes.stylize = function (str, styleName) {
    var style = chalk.styles[styleName] || chalk.styles.white;
...
function stringifyObject(obj, options, level) {
    ...
    if (out.length === 0) {
        // Could not get any keys to print nicely, just return the string representation of the object
        return obj.toString();
    } else {
        return "{" + ws
                   + out.join(',' + (pretty ? ws : ' '))
